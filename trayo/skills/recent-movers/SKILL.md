...
2. To watch future changes, create a `job_change` signal and use the `scan-for-signal` skill. While the run is active, keep polling it. After it settles, read `run.error` and `run.blockedSignals`. Follow the recovery steps in `scan-for-signal` for `account_not_ready` or `company_not_found`.
