...
**Call this before anything else below.** It is not retroactive — an account added or an
event produced before it's set keeps searching on seniority alone / stays people-less, even
after you set it later.
...
`reasonCode: "duplicate"` skip too; treat it the same as a created account.
...
`lookbackDays: 30`, `waitSeconds: 45`. At most 200 accounts and 10 signals per call — split into
...
The stakeholder definition and solutions you set, the account cohort (target vs. event-bearing
vs. padded, plus any surplus left over from the over-search), the signal(s) created, the
discovery run id(s) with `eventsNew`, and suggested next steps — `trayo_list_events` for more
detail, `trayo_search_stakeholders` for people at accounts that still show `people: []`.
