...
4. Run `trayo_run_discovery` `{ accountIds, signalKeys: [signalKey], lookbackDays, waitSeconds: 45 }`; while `settled` is false, `trayo_get_discovery` `{ runId, waitSeconds: 45 }`. A run is final only when `settledAt` is set. Use at most 200 accounts per run so `blockedSignals` can return every affected account ID. Split larger scans into batches with the same `signalKeys` and `lookbackDays`, and wait for each run to settle before starting the next. `lookbackDays` also sets how hard the search digs, so a long window over many accounts is not a fast call.
